...
RSpec.describe SendEventEmailsJob, type: :job do
  ...
  describe '#perform' do
    ...
    context 'with no errors being raised' do
      ...
      let!(:subscription4) { create(:event_subscription_comment_for_project, receiver_role: 'maintainer', user: nil, group: group, channel: 'rss') }
      ...
      it "creates an rss notification for user's email" do
        notification = Notification::RssFeedItem.find_by(subscriber: user)
      ...
      it "creates an web notification for user's email" do
        notification = Notification::WebItem.find_by(subscriber: user)
