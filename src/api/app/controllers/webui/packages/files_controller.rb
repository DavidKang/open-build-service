  ...
  module Packages
    class FilesController < Packages::MainController
      ...
      def create
        ...
        upload_file = FilesManager::UploadFiles.new(@package, params[:files], params[:file_urls], params[:files_new], params[:comment]).call
        if upload_file.valid?
          flash[:success] = "'#{upload_file.list.to_sentence}' has been successfully uploaded."
        else
          flash[:error] = "Error while uploading '#{upload_file.list.to_sentence}' file: #{upload_file.errors.compact.join("\n")}."
        end
