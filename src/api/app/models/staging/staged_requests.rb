class Staging::StagedRequests
  ...
  def stage_request(request)
    ...
    if bs_request_action.is_submit?
      return unless package_can_be_linked?(request, bs_request_action.target_package)
  ...
  def package_can_be_linked?(request, target_package)
    return true unless Package.find_by(project: staging_project, name: target_package)

    errors << "Can't stage request '#{request.number}': package '#{target_package}' already exists in '#{staging_project}'."
    return false
  end
end
